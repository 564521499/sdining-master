...
class IndexView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get('position', '') == '1':  # 获取食堂名称 1,2,3
            context['business_list'] = context['business_list'].filter(position=1).order_by('-sort_weight')  # 商店权重排序列表
            print(context['business_list'])
            context['recommend_list'] = context['recommend_list'].filter(business__position=1).order_by('-business__sort_weight') # 推荐的商家
            print(context['recommend_list'])
